...
class SuchaiState(SatState):
    ...
    def process_request(self, message: bytes) -> bytes:
        ...
        elif message[0] == SatellitePersonality.SIM_COMM_ID:
            reply = self.process_comm_request(message)
        else:
            reply = message
        return reply
    ...
    def process_comm_request(self, message: bytes) -> bytes:
        if message[1] == 0x00:
            is_reachable = self.subsystem_states["comm"].read_value('is_comm_reachable').value
            reply = struct.pack('B', 0xFF if is_reachable else 0x00)
        else:
            reply = message
        return reply
