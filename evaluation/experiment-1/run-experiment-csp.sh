...
DEFAULT_MINUTES_AHEAD="9"

usage() {
    cat <<EOF
Usage: $(basename "$0") [NORAD_NUMBER MINUTES_AHEAD]

Run experiment for HoneySat using docker compose.
...
COMPOSE_FILE="${SCRIPT_DIR}/docker-compose-csp.yaml"
...
DOCKER_COMPOSE_CMD=(docker compose --file "${COMPOSE_FILE}" --profile ALL)
...
echo "Pulling docker images (profile: ALL)..."
"${DOCKER_COMPOSE_CMD[@]}" pull

# --- Build / prepare Docker images ------------------------------------------
echo "Building Docker images (profile: ALL)..."
